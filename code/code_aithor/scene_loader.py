...
class THORDiscreteEnvironment(object):
  ...
  def __init__(self, config=dict()):
    ...
    self.path_history = []
    self.obs_history = []  # stocker les images
    self.reset()
  ...
  def reset(self):
    ...
    # Premier point de l'épisode : on l'ajoute dès le reset pour que
    # path_history[0] existe toujours avec sa rotation, même avant le
    # premier step() (nécessaire pour animate_episode / start_rotation).
    self.path_history.append({
        'x': self.locations[self.current_state_id][0],
        'y': self.locations[self.current_state_id][1],
        'rotation': self.rotations[self.current_state_id],
        'is_terminal': False
    })
    self.obs_history.append(
        self.h5_file['observation'][self.current_state_id]
    )
