...
# Verrou global : matplotlib n'est pas thread-safe, un seul thread à la fois
# peut créer/manipuler une figure. Sans ça, des threads concurrents créent
# une accumulation de figures non fermées -> fuite mémoire -> OOM.
_ANIMATION_LOCK = threading.Lock()

# Fréquence de génération des GIFs : avec 100 threads, générer un GIF tous
# les 20 épisodes par thread est beaucoup trop fréquent (potentiellement
# plusieurs dizaines de GIFs simultanés). On espace fortement.
ANIMATION_EVERY_N_EPISODES = 500
...
class A3CTrainingThread(object):
    def __init__(self,
        ...
        # Réseau local : une seule scène (scene_scope)
        ...
        # --- FIX GRADIENT : mapping nom-normalisé local -> global ---
        # Le réseau global a des couches scene-specific pour TOUTES les scènes,
        # alors que le réseau local n'en a que pour scene_scope.
        # On construit une correspondance par nom normalisé pour appliquer
        # les gradients uniquement aux bons paramètres globaux.
        self.local_to_global_map = self._build_param_map()
    ...
    def _normalize_param_name(self, name):
        """Normalise un nom de paramètre en retirant le scope de scène,
        pour pouvoir faire correspondre les paramètres locaux (1 scène)
        aux paramètres globaux (toutes les scènes)."""
        # Retire le nom de la scène locale du nom de paramètre
        return name.replace(self.scene_scope, "SCENE")

    def _build_param_map(self):
        """Construit une liste parallèle aux paramètres locaux, donnant
        l'index du paramètre global correspondant (ou None si le paramètre
        global n'existe pas / n'est pas partagé, ex: couches d'une autre scène)."""
        global_named_params = list(self.global_network.named_parameters())
        local_named_params  = list(self.local_network.named_parameters())

        # Table de correspondance nom_normalisé -> index dans global
        global_lookup = {}
        for idx, (name, _) in enumerate(global_named_params):
            norm_name = self._normalize_param_name(name)
            global_lookup[norm_name] = idx

        mapping = []
        unmatched = []
        for name, _ in local_named_params:
            norm_name = self._normalize_param_name(name)
            global_idx = global_lookup.get(norm_name, None)
        ...
        if VERBOSE and unmatched:
            sys.stdout.write(
                "[thread #%d] Attention : %d paramètres locaux sans correspondance globale : %s\n"
                % (self.thread_index, len(unmatched), unmatched)
            )
    ...
    def process(self, global_t, summary_writer=None):
        ...
        for i in range(LOCAL_T_MAX):
            ...
            if terminal:
                ...
                # Animation très espacée (toutes les ANIMATION_EVERY_N_EPISODES
                # épisodes), protégée par un verrou car matplotlib n'est pas
                # thread-safe. Sans verrou + sans espacement -> OOM garanti
                # avec 100 threads concurrents.
                self.episode_count += 1
                if self.episode_count % ANIMATION_EVERY_N_EPISODES == 0:
                    ...
                    with _ANIMATION_LOCK:
                        animate_episode(
                            ...
                            self.scene_scope,
                            save_path,
                            target_rotation=self.env.rotations[int(self.task_scope)],
                            start_rotation=self.env.path_history[0]['rotation'] if self.env.path_history else 0
                        )
        ...
        # --- APPLICATION DES GRADIENTS via le mapping local -> global ---
        local_grads = self.trainer.get_accum_grad_list()
        global_params = list(self.global_network.parameters())

        mapped_global_params = []
        mapped_grads = []
        for grad, global_idx in zip(local_grads, self.local_to_global_map):
            if global_idx is not None:
                mapped_global_params.append(global_params[global_idx])
                mapped_grads.append(grad)
            # si global_idx est None, on ignore ce gradient (pas de
            # correspondance globale, ex: paramètre orphelin)
